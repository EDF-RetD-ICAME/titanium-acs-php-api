...
class User extends Base
{
  ...
  const URL_LOGOUT = 'users/logout.json';
  const URL_ME = 'users/show/me.json';
  ...
  {
    return substr(sha1($this->password), 0, 20);
  }
  ...
  {
    $response = $this->get($this->getFullUrl(self::URL_LOGOUT));
  ...
  {
    $response = $this->get($this->getFullUrl(self::URL_ME));
  ...
  public function subscribe($device_token, $channel = null, $type = null)
  {
    ...
    return $notification->subscribe($device_token, $channel, $type);
  }
  ...
  protected function addParameter(&$parameters, $field)
  {
    if ($this->$field)
    {
      $method = lcfirst(\sfInflector::camelize('get_'.$field));

      if (method_exists($this, $method))
      {
        $parameters[$field] = $this->$method();
      }
    }
  }
}